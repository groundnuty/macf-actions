...
A caller supplying **neither** the bundle nor a complete six fails **at the very first job (`pick-runner`), before any other job does any work** (`groundnuty/macf-actions#82`) — naming both valid forms and exactly which secret(s) are missing from the six, rather than proceeding on empty values. This earliest check covers issue-comment, PR, PR-review, and CI-completion events; **`issues: labeled` events are handled by `route-by-label`'s own check instead** (deliberately — an earliest-job check can't yet tell an agent label from a non-agent one, and failing on every non-agent label would be worse than today). If you're a **cross-org/enterprise caller**, the failure names the likely cause: `secrets: inherit` only propagates within the same GitHub organization/enterprise, so having all six secrets on your own repo does not deliver them here — pass them explicitly, or supply `MACF_ROUTING_BUNDLE` (`groundnuty/macf#1338`). A caller supplying a **malformed** bundle (bad base64, bad JSON, or missing a key) also fails loudly and does not silently fall back to the six even if those happen to be present too. (Each `route-by-*` job also still validates independently before it uses the secrets — the `pick-runner` check is additive, not a replacement, and remains the sole enforcement point for `route-by-label`.)
