...
- **Self-hosted-runner enablement for the routing workflow (#49).** `runs-on` is now parameterized per consumer repo via the `MACF_ROUTING_RUNS_ON` variable (unset → `ubuntu-latest`, behavior unchanged; set to a JSON array of runner labels, e.g. `["self-hosted","macf-vm"]`, to run routing on a VM-local self-hosted runner — `fromJSON` parses it). On a self-hosted runner the Tailscale `Connect` + readiness-poll steps are skipped (`if: runner.environment != 'self-hosted'`): the VM is already on the tailnet via its own tailscaled, so the routing SSH reaches the agent without the hosted-runner join — removing the ~22s Tailscale tax. The existing SSH send path serves **both** arms (the AC-accepted lower-diff inject; a separate no-SSH direct-helper inject can follow). Inert by default on `ubuntu-latest`; depends on the #47 prompt-injection hardening (merged) as its security prerequisite.
